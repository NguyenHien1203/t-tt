...
export class CapNhatComponent {
    ...
    public async BindDialogData() {
        try {
            const data = await this.service.getNhomQuyenById(this.id);
    ...
    public CapNhat() {
        ...
        if (this.formCapNhat.valid) {
            ...
            let nhomNguoiDung = {
                id: itemData.id,
                tenNhomQuyen: itemData.tenNhomQuyen,
                moTa: itemData.moTa,
                lastModified: new Date(),
                lastModifiedBy: this.idUser,
            };

            this.service.capNhatNhomQuyen(nhomNguoiDung).subscribe(
                (data) => {
                    if (data.isError) {
