...
@Component({
    selector: 'app-lich-co-quan',
    templateUrl: './lich-co-quan.component.html',
    styleUrls: ['./lich-co-quan.component.scss'],
    providers: [MessageService, ConfirmationService, DatePipe],
})
export class LichCoQuanComponent {
    constructor(
    ...
    public XuLyLich(id: string, action: string): void {
        this.confirmService.confirm({
            message: 'Bạn có chắc chắn xác nhận?',
            header: 'Xác nhận',
            icon: 'pi pi-info-circle',
            accept: () => {
                this.service.xuLyLichCoQuan(id, action).subscribe(
                    (data) => {
                        if (data.isError) {
    ...
    public Thoat(itemHt: any, loai: string): void {
        ...
        if (loai === 'I') this.hienThiImport = false;
        this.LoadDanhSach();
    }
    ...
    public Xoa(id: string) {
        this.confirmService.confirm({
            ...
            icon: 'pi pi-info-circle',
            accept: () => {
                this.service.xoaLichCoQuan(id).subscribe(
                    (data) => {
                        if (data.isError) {
                            this.messageService.add({
                                severity: 'error',
