...
export class CapNhatComponent {
  ...
  public async BindDataDialog() {
      try {
          const data = await this.service.getLichHDNDVaDBQHById(this.id);
  ...
  public CapNhat(): void {
      ...
      if (this.formCapNhat.valid) {
          let lichHDNDvaDBND = this.formCapNhat.value;
          this.service.capNhatLichHDNDVaDBQH(lichHDNDvaDBND).subscribe(
              (data) => {
                  let resData = data;
                  if (resData.isError) {
