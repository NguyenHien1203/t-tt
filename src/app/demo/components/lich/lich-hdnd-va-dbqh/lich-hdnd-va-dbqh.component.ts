...
import { LichHdndVaDbqhService } from 'src/app/demo/service/lich/lich-hdnd-va-dbqh.service';
import { TimKiemLichHDNDVaDBQH } from 'src/app/models/thong-tin-khac/lich/lich-hdnd-va-dbnd';

@Component({
  selector: 'app-lich-hdnd-va-dbqh',
  templateUrl: './lich-hdnd-va-dbqh.component.html',
  styleUrls: ['./lich-hdnd-va-dbqh.component.scss'],
  providers : [ConfirmationService, MessageService, DatePipe]
})
export class LichHdndVaDbqhComponent {
  constructor(
    private messageService: MessageService,
    private service: LichHdndVaDbqhService,
...
lstLichCaNhan: any[] = [];
items = [{ label: 'Lịch' }, { label: 'Lịch HĐND & ĐBQH' }];
home = { icon: 'pi pi-home', routerLink: '/' };
timKiemDanhSach: TimKiemLichHDNDVaDBQH = {
    tuNgay: '1901-01-01',
    denNgay: '1901-01-01',
...
public LoadDanhSach(): void {
    ...
    this.service
        .getDanhSachLichHDNDVaDBQH(this.timKiemDanhSach)
        .then((data) => {
...
public Xoa(id: string) {
    this.confirmService.confirm({
        message: 'Bạn có chắc chắn xác nhận xóa lịch uỷ ban nhân dân tỉnh?',
        header: 'Xác nhận',
        icon: 'pi pi-info-circle',
        accept: () => {
            this.service.xoaLichHDNDVaDBQH(id).subscribe(
                (data) => {
                    if (data.isError) {
