...
import { LichTinhUyService } from 'src/app/demo/service/lich/lich-tinh-uy.service';
import { TimKiemLichTinhUy } from 'src/app/models/thong-tin-khac/lich/lich-tinh-uy';

@Component({
  selector: 'app-lich-tinh-uy',
  templateUrl: './lich-tinh-uy.component.html',
  styleUrls: ['./lich-tinh-uy.component.scss'],
  providers:[MessageService, ConfirmationService, DatePipe]
})
export class LichTinhUyComponent {
  constructor(
    private messageService: MessageService,
    private service: LichTinhUyService,
...
lstLichCaNhan: any[] = [];
items = [{ label: 'Lịch' }, { label: 'Lịch tỉnh ủy' }];
home = { icon: 'pi pi-home', routerLink: '/' };
timKiemDanhSach: TimKiemLichTinhUy = {
    tuNgay: '1901-01-01',
    denNgay: '1901-01-01',
...
public LoadDanhSach(): void {
    ...
    this.service
        .getDanhSachLichTinhUy(this.timKiemDanhSach)
        .then((data) => {
...
public Xoa(id: string) {
    this.confirmService.confirm({
        message: 'Bạn có chắc chắn xác nhận xóa lịch tỉnh ủy?',
        header: 'Xác nhận',
        icon: 'pi pi-info-circle',
        accept: () => {
            this.service.xoaLichTinhUy(id).subscribe(
                (data) => {
                    if (data.isError) {
