...
@Component({
  ...
  styleUrls: ['./tiep-nhan.component.scss']
})
export class TiepNhanComponent {
  ...
  public async BindDataDialog() {
    try {
      const data = await this.service.GetVanBanTiepNhan(this.id);
  ...
  public Submit() {
    ...
    if (this.formThongTinVanBan.valid) {
      ...
      this.formTT_VB_fomat.vanBanId = this.id;
