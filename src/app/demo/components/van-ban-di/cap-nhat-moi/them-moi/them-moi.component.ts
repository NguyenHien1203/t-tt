...
export class ThemMoiComponent {
  ...
  items: any[] = [{ label: 'Văn bản đi' }, { label: 'Cập nhật mới' }, { label: 'Thêm mới văn bản' }];
  ...
  public LoadPhongBanSoanThao() {
    this.service.getDanhSachPhongBan(this.idDonViLamViec, this.authService.GetmUserInfo()?.userName ?? "").then(data => {
  ...
  public ChangeSoVanBan(event) {
    this.service.getSoHienTai(event.toString(), this.formThongTinVanBan.value.ngayBanHanh, "0", this.formThongTinVanBan.value.soDi, "0").then(data => {
      this.formThongTinVanBan.patchValue({
        soHienTai: data,
      });
    }).then(() => {
      this.service.getSoKiHieu(event, "0", this.formThongTinVanBan.value.soHienTai).then(data => {
        this.formThongTinVanBan.patchValue({
          soKiHieu: data,
        });
      });
  ...
  public ChangeLoaiVanBan(event) {
    this.service.getSoKiHieu(this.formThongTinVanBan.value.soVanBanId, event, this.formThongTinVanBan.value.soHienTai).then(data => {
      this.formThongTinVanBan.patchValue({
        soKiHieu: data,
      });
    });
  ...
  public ThemMoi() {
    ...
    if (this.formThongTinVanBan.valid) {
      this.formThongTinVanBan.value.donViId = this.authService.GetDonViLamViec();
      ...
      this.service.themMoiVanBanDi(this.vanBanDi).subscribe(data => {
        if (data.isError) {
