...
@Component({
  selector: 'app-thu-hoi',
  templateUrl: './thu-hoi.component.html',
  styleUrls: ['./thu-hoi.component.scss']
})
export class ThuHoiComponent {
  ...
  public ThuHoiVanBan(): void {
      ...
      if (this.lstDonViNhan.length == 0) {
          this.messageService.add({
              severity: 'error',
              summary: 'Error',
              detail: 'Yêu cầu chọn đơn vị thu hồi',
          });
          return;
      }
      let itemData: any = {
      ...
      this.service.thuHoiVanBan(itemData).subscribe(
          (data) => {
              if (data.isError) {
                  this.messageService.add({
                      severity: 'error',
