...
export class CapNhatVanBanComponent implements OnInit {
  ...
  GetDSLanhDao() {
    this.quanTriVanBanDiService.getDanhSachLanhDaoKy(this.idDonViLamViec)
      .subscribe(data => {
        if (data.isError) {
          this.msgs = [];
          this.msgs.push({ severity: 'error', detail: "Không tìm thấy dữ liệu" })
  ...
  GetDSPhongBanSoanThao() {
    this.quanTriVanBanDiService.getDanhSachPBSoanThao(this.idDonViLamViec, this.userNameAd)
      .subscribe(data => {
        if (data.isError) {
          this.msgs = [];
          this.msgs.push({ severity: 'error', detail: "Không tìm thấy dữ liệu" })
  ...
  DownloadFile(filePath: string, fileName: string) {
    ...
    this.uploadFileService.downloadFile(filePath, fileName, url).subscribe((data) => {
  ...
  public async GetVanBanById() {
    try {
      const data = await this.quanTriVanBanDiService.getVanBanDiCapNhat(this.idVb);
      ...
      this.formVanBan.patchValue({
        ...
        phongBanSoanThaoId: this.lstPBSoanThao
          .filter((pdId) => pdId.value.split('%')[0] == itemData.phongBanSoanThaoId)
          .map((pdId) => pdId.value).join(', ') ?? '',
        lanhDaoKyId: this.lstLanhDao
          .filter((ldId) => ldId.value.split('%')[0] == itemData.lanhDaoKyId)
          .map((ldId) => ldId.value).join(', ') ?? '',
