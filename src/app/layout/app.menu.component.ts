...
export class AppMenuComponent implements OnInit {
     ...
     */
    public GetDataMenu() {
        const Userid = this.authService.GetmUserInfo().userId;
        this.taikhoanService.GetDataMenu(Userid).subscribe(data => {
            if (data.isError) {
                console.log("Dữ liệu không hợp lệ")
            } else {
                this.LstMenuCon = data.objData;
                const filteredItems = this.filterItems(this.model);
                const objHome =  {
                         label: '',
                        items: [
    ...
    public filterItems(model: any): any {
        let filteredItems = {
            label: '',
            code: '0',
            items: []
        };
        model[0].items.forEach(item => {
